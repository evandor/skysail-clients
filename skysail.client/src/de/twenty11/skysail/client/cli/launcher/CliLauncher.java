 ...
 */
package de.twenty11.skysail.client.cli.launcher;
...
/**
 * This is the entry point of the entire clamshell cli container (Main). This is
 * a thin starter module that serves as a bootloader for the system.
 * <ul>
...
public class CliLauncher {
	public static void main(String[] args) throws Exception {
		...
		String pluginsDirName = propsMap
				.get(Configurator.KEY_CONFIG_PLUGINSDIR);

		// only continue if plugins are found
		File libDir = new File(libDirName);
		if (!libDir.exists()) {
			System.out
					.printf("%nLib directory [%s] not found. ClamShell-Cli will exit.%n%n",
		...
		ClassLoader cl = Clamshell.Runtime.createClassLoaderForPath(
				new File[] { libDir }, parent);
		Thread.currentThread().setContextClassLoader(cl);

		File pluginsDir = new File(pluginsDirName);
		if (!pluginsDir.exists()) {
			System.out
					.printf("%nPugins directory [%s] not found. ClamShell-Cli will exit.%n%n",
		...
		if (context.getPlugins().size() > 0) {
			...
			if (context.getShell() != null) {
				...
				System.out.printf(
						"%nNo Shell component found in plugins directory [%s]."
								+ " ClamShell-Cli will exit now.%n",
						pluginsDir.getCanonicalPath());
				System.exit(1);
			}
		} else {
			System.out
					.printf("%nNo plugins found in [%s]. ClamShell-Cli will exit now.%n%n",
							pluginsDir.getCanonicalPath());
			System.exit(1);
		}
	}

//	private static Shell getShell(Context context) {
//		return context
//				.getPluginsByType(Shell.class)
//				.stream()
//				.filter(s -> !s.getClass().getName()
//						.equals(MockShell.class.getName())).findFirst()
//				.orElse(null);
//	}
}
